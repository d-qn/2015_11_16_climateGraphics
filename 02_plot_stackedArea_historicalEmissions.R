...
maxY <- data %>% group_by(Year) %>% summarise(tot = sum(`Total CO2 Emissions Excluding Land-Use Change and Forestry (MtCO2)`)) %>% select(tot) %>% max
...
for (i in 1:ncol(trad)) {
  ...
  output2.html <- paste("output/02_CO2emissions_stackedAreaPerc_", lang, ".html", sep ="")
  ...
  dd$name2 <- paste0(
    ...
    '<tr><td align="left" colspan="2"><div style="font-size:0.9em">', dd$Country,'</td><td></td></tr>',
    '<tr><td colspan="3"><div style="font-size:0.85em"><b>',
    round(dd$`cumulativeShare`, 2), "</b> %", '</div></td></tr>',
    '</table>')
  ...
  ### % stacked percentages
  b <- Highcharts$new()
  b$chart(zoomType = "xy", type = 'area', height = chartHeight, spacing = 5)
  hSeries <- hSeries2(data.frame(
    x = dd$Year,
    y = dd$`Total CO2 Emissions Excluding Land-Use Change and Forestry (MtCO2)`,
    name = dd$name2,
  ...
  b$colors(swi_pal)
  b$plotOptions(area = list(stacking = "percent",
    lineWidth = 0, marker = list(enabled = FALSE, symbol = "circle", radius = 1)),
    series = list(fillOpacity = 1, trackByArea = TRUE)
  )

  b$legend(borderWidth= 0, itemMarginTop = 3, itemMarginBottom = 5, itemHoverStyle = list(color = '#996666'),
    itemStyle = list(fontWeight = "normal", fontSize = "0.8em"),
    title = list(style = list(fontWeight ='normal'),
  ...
  b$xAxis(title = list(text = ""), max = max(dd$Year), min = min(dd$Year))

  b$lang( numericSymbols= NULL)
  b$yAxis(title = list(text = ""),
    max = 100,
    gridLineColor = "#EFEFEF",
    labels = list(formatter = "#! function () {return this.value + '%';} !#"))

  b$tooltip(formatter = "#! function() { return this.point.name; } !#", useHTML = T ,
    borderWidth = 2, style = list(padding = 4))

  hChart2.html <- tempfile("hChart_area2")
  b$save(hChart2.html)

  # Convert highcharts-rCharts html chart into a responsive one
  hChart2responsiveHTML(hChart2.html, output.html = output2.html,
    h2 = trad['title2',lang], descr = trad['descr',lang],
    source = paste0(trad["source",lang], ": ",
    htmlLink("http://www.wri.org/resources/data-sets/cait-historical-emissions-data-countries-us-states-unfccc",
    trad["sourceName",lang])),
    h3 = "",
